...
private fun convertDuration(isoString: String): String {
    fun toString(days: Long, hours: Long, minutes: Long, seconds: Long): String =
        when {
            days > 0 ->
                "${days.timeString}:${hours.timeString}:${minutes.timeString}:${seconds.timeString}"
    ...
    if (matcherDay.matches()) {
        return toString(
            matcherDay.group(1).toLong(),
            matcherDay.group(2).toLong(),
            matcherDay.group(3).toLong(),
            matcherDay.group(4).toLong()
        )
    }
    ...
    if (matcherHour.matches()) {
        return toString(
            0L,
            matcherHour.group(1).toLong(),
            matcherHour.group(2).toLong(),
            matcherHour.group(3).toLong()
        )
    }
