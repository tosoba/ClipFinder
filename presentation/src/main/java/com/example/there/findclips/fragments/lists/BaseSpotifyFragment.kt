...
abstract class BaseSpotifyFragment<T : Parcelable> : Fragment() {
    ...
    override fun onInflate(context: Context?, attrs: AttributeSet?, savedInstanceState: Bundle?) {
        ...
        val attributes = activity?.obtainStyledAttributes(attrs, R.styleable.BaseSpotifyFragment)
        attributes?.getText(R.styleable.BaseSpotifyFragment_main_hint_text)?.let { viewState.mainHintText.set(it.toString()) }
        attributes?.getText(R.styleable.BaseSpotifyFragment_additional_hint_text)?.let { viewState.additionalHintText.set(it.toString()) }
        attributes?.recycle()
    }
    ...
    companion object {
        ...
        fun <T, I> putArguments(fragment: T, mainHintText: String, additionalHintText: String) where T : BaseSpotifyFragment<I> {
