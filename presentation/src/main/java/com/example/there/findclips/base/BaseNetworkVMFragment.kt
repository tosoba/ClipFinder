...
abstract class BaseNetworkVMFragment<T : BaseViewModel> : BaseVMFragment<T>() {
    ...
    private fun showNoConnectionDialog() = context?.let {
        dialog = AlertDialog.Builder(it)
                ...
                .setNegativeButton(getString(R.string.cancel), { dialog, _ ->
                    Toast.makeText(it, R.string.internet_connection_is_needed, Toast.LENGTH_LONG).show()
                    dialog.dismiss()
                    isDialogShowing = false
                })
                .create()
