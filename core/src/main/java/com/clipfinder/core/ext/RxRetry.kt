...
class WithVariableDelay(attempts: Long, val unit: TimeUnit, val getDelay: (Int) -> Long) :
    RetryStrategy(attempts)

fun <T> Single<T>.retry(strategy: RetryStrategy): Single<T> =
    when (strategy) {
...
fun <T> Flowable<T>.retry(strategy: RetryStrategy): Flowable<T> =
    when (strategy) {
        is Times -> retry(strategy.attempts)
        is WithDelay ->
            retryWhen { errors ->
...
fun <T> Observable<T>.retry(strategy: RetryStrategy): Observable<T> =
    when (strategy) {
        is Times -> retry(strategy.attempts)
        is WithDelay ->
            retryWhen { errors ->
...
fun Completable.retry(strategy: RetryStrategy): Completable =
    when (strategy) {
        is Times -> retry(strategy.attempts)
        is WithDelay ->
            retryWhen { errors ->
