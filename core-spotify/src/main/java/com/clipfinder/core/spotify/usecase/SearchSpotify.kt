...
class SearchSpotify(private val repo: ISpotifyRepo) :
    UseCaseWithArgs<SearchSpotify.Args, Single<Resource<SpotifySearchResult>>> {
    override fun run(args: Args): Single<Resource<SpotifySearchResult>> =
        when (args) {
    ...
    sealed class Args {
        ...
        class More(override val query: String, val offset: Int, val type: SpotifySearchType) :
            Args()
    }
}
