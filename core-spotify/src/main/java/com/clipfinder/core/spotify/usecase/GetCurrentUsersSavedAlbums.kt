...
class GetCurrentUsersSavedAlbums(private val repo: ISpotifyRepo) :
    UseCaseWithArgs<Int, Single<Resource<Paged<List<ISpotifySimplifiedAlbum>>>>> {
    override fun run(args: Int): Single<Resource<Paged<List<ISpotifySimplifiedAlbum>>>> =
        repo.getCurrentUsersSavedAlbums(offset = args)
}
