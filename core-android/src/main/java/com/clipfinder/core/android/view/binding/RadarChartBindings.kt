...
fun bindRadarChartView(radarChart: RadarChart, view: RadarChartView) =
    with(radarChart) {
        ...
        marker = view.markerView.apply { chartView = this@with }
        ...
        with(xAxis) {
            ...
            view.xAxisView.valueFormatter?.let { this.valueFormatter = it }
        ...
        with(yAxis) {
            ...
            view.yAxisView.valueFormatter?.let { this.valueFormatter = it }
...
fun bindRadarChartData(radarChart: RadarChart, data: RadarData?): Unit =
    with(radarChart) {
        data?.let {
            setData(it)
            invalidate()
