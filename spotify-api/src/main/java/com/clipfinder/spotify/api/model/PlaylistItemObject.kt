 ...
 * @param availableMarkets A list of the countries in which the track can be played, identified by
 ...
 * @param explicit Whether or not the track has explicit lyrics ( true = yes it does; false = no it
 * does not OR unknown).
 * @param externalIds
 * @param externalUrls
 * @param href A link to the Web API endpoint providing full details of the track.
 * @param id The Spotify ID for the track.
 * @param isPlayable Part of the response when Track Relinking is applied. If true , the track is
 * playable in the given market. Otherwise false.
 * @param linkedFrom
 * @param name The name of the track.
 * @param popularity The popularity of the track. The value will be between 0 and 100, with 100
 * being the most popular. The popularity of a track is a value between 0 and 100, with 100 being
 * the most popular. The popularity is calculated by algorithm and is based, in the most part, on
 * the total number of plays the track has had and how recent those plays are. Generally speaking,
 * songs that are being played a lot now will have a higher popularity than songs that were played a
 ...
 * that the popularity value may lag actual popularity by a few days: the value is not updated in
 * real time.
 * @param previewUrl A link to a 30 second preview (MP3 format) of the track. Can be null
 * @param restrictions Part of the response when Track Relinking is applied, the original track is
 * not available in the given market, and Spotify did not have any tracks to relink it with. The
 * track response will still contain metadata for the original track, and a restrictions object
 * containing the reason why the track is not available: \"restrictions\" : {\"reason\" :
 * \"market\"}
...
data class TrackObject(
    @Json(name = "album") override val album: SimplifiedAlbumObject,
    ...
    @Json(name = "duration_ms") override val durationMs: Int,
    ...
    @Json(name = "is_playable") val isPlayable: Boolean? = null,
    @Json(name = "linked_from") val linkedFrom: LinkedTrackObject? = null,
    ...
    @Json(name = "popularity") override val popularity: Int,
    /* A link to a 30 second preview (MP3 format) of the track. Can be null */
    @Json(name = "preview_url") override val previewUrl: String? = null,
    /* Part of the response when Track Relinking is applied, the original track is not available in the given market, and Spotify did not have any tracks to relink it with. The track response will still contain metadata for the original track, and a restrictions object containing the reason why the track is not available: \"restrictions\" : {\"reason\" : \"market\"} */
    @Json(name = "restrictions") val restrictions: List<TrackRestrictionObject>? = null,
    /* The number of the track. If an album has several discs, the track number is the number on the specified disc. */
    @Json(name = "track_number") override val trackNumber: Int,
    /* The Spotify URI for the track. */
    @Json(name = "uri") override val uri: String
) : PlaylistItemObject(PlaylistItemType.track), ISpotifyTrack

/**
 *
 * @param audioPreviewUrl A URL to a 30 second preview (MP3 format) of the episode. null if not
 * available.
 * @param description A description of the episode.
 * @param durationMs The episode length in milliseconds.
 * @param explicit Whether or not the episode has explicit content (true = yes it does; false = no
 ...
 * @param language Note: This field is deprecated and might be removed in the future. Please use the
 * languages field instead. The language used in the episode, identified by a ISO 639 code.
 ...
 * @param releaseDate The date the episode was first released, for example \"1981-12-15\". Depending
...
data class EpisodeObject(
    ...
    @Json(name = "is_externally_hosted") val isExternallyHosted: Boolean,
    ...
    @Json(name = "resume_point") val resumePoint: ResumePointObject,
