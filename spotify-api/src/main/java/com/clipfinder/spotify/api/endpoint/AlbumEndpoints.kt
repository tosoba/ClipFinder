...
interface AlbumEndpoints {
    /**
     * Get an Album Get Spotify catalog information for a single album. Responses:
     * - 200: On success, the HTTP status code in the response header is 200 OK and the response
     * body contains an album object in JSON format. On error, the header status code is an error
    ...
    ): Single<NetworkResponse<AlbumObject, ErrorResponse>>

    /**
     * Get an Album&#39;s Tracks Get Spotify catalog information about an album’s tracks. Optional
     * parameters can be used to limit the number of tracks returned. Responses:
     * - 200: On success, the HTTP status code in the response header is 200 OK and the response
     * body contains an album object in JSON format. On error, the header status code is an error
     * code and the response body contains an error object.
     * - 0: Unexpected error
     ...
     * (optional)
     * @param offset The index of the first track to return. Default: 0 (the first object). Use with
     * limit to get the next set of tracks. (optional)
    ...
    fun getAnAlbumsTracks(
        @Header("Authorization") authorization: String? = null,
        @Path("id") id: String,
        @Query("limit") limit: Int? = null,
        @Query("offset") offset: Int? = null,
        @Query("market") market: String? = null
    ): Single<NetworkResponse<SimplifiedTracksPagingObject, ErrorResponse>>

    /**
     * Get Multiple Albums Get Spotify catalog information for multiple albums identified by their
     * Spotify IDs. Responses:
     * - 200: On success, the HTTP status code in the response header is 200 OK and the response
     * body contains an object whose key is \"albums\" and whose value is an array of album objects
     ...
     */
    @GET("albums")
    fun getMultipleAlbums(
        @Header("Authorization") authorization: String? = null,
        @Query("ids") ids: String,
        @Query("market") market: String? = null
    ): Single<NetworkResponse<AlbumsObject, ErrorResponse>>
}
