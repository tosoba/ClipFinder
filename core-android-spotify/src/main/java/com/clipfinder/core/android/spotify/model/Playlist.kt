...
) : Parcelable, ISpotifySimplifiedPlaylist, NamedImageListItem {

    constructor(
        other: ISpotifySimplifiedPlaylist
    ) : this(
        other.id,
        other.name,
        other.uri,
...
fun Playlist.clickableListItem(itemClicked: () -> Unit): ImageListItemBindingModel_ =
...
fun Playlist.clickableGridListItem(itemClicked: () -> Unit): GridPlaylistItemBindingModel_ =
    GridPlaylistItemBindingModel_().id(id).playlist(this).itemClicked { _ -> itemClicked() }
