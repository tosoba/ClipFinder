...
package com.example.spotifyapi.service.http
...
abstract class SpotifyEndpoint(val api: SpotifyAPI) {
    ...
    private fun createConnection(
        ...
        HttpConnection(
                ...
                body,
                contentType,
                listOf(HttpHeader("Authorization", "Bearer ${it.accessToken}")),
                api = api
        )
...
data class CacheState(val data: String, val eTag: String?, val expireBy: Long = 0) {
    ...
    internal fun update(expireBy: String): CacheState {
        ...
        val time = group?.getOrNull(1)?.toLongOrNull()
...
internal fun String.byteEncode(): String = Base64.encodeToString(toByteArray(), Base64.NO_WRAP)
