...
package com.example.spotifyapi.service.models.serialization

import com.example.spotifyapi.service.SpotifyAPI
import com.example.spotifyapi.service.SpotifyException
...
internal inline fun <reified T : Any> String.toObjectNullable(o: SpotifyAPI?): T? = try {
    toObject(o)
} catch (e: Exception) {
    null
}

internal inline fun <reified T : Any> String.toObject(o: SpotifyAPI?): T {
...
internal inline fun <reified T> String.toPagingObject(
    ...
    val adapter = endpoint.api.moshi.adapter<PagingObject<T>>(pagingType)
...
internal inline fun <reified T> String.toInnerArray(innerName: String): List<T> {
    ...
    return (map[innerName]
            ?: error("Inner object with name $innerName doesn't exist in $map")).toList()
}
