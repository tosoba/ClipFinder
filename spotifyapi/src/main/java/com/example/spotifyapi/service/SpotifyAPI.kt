...
import com.example.spotifyapi.service.endpoints.client.*
import com.example.spotifyapi.service.endpoints.public.*
import com.example.spotifyapi.service.http.*
import com.example.spotifyapi.service.models.AuthenticationError
import com.example.spotifyapi.service.models.BadRequestException
import com.example.spotifyapi.service.models.Token
import com.example.spotifyapi.service.models.TokenValidityResponse
 ...
 * @property albums Provides access to Spotify [album endpoints](https://developer.spotify.com/documentation/web-api/reference/items/)
...
abstract class SpotifyAPI internal constructor(
    ...
    val expireTime: Long
        get() = token?.let { System.currentTimeMillis() + it.expiresIn * 1000 }
    ...
    fun isTokenValid(makeTestRequest: Boolean = true): TokenValidityResponse {
        token?.let {
            if (!it.shouldRefresh()) return TokenValidityResponse(false, SpotifyException("Token expired"))
        ...
        } ?: return TokenValidityResponse(false, SpotifyException("No token"))
    }
}
...
class SpotifyAppAPI internal constructor(
    ...
    override fun refreshToken(): Token {
        if (clientId != "not-set" && clientSecret != "not-set") {
            return getCredentialedToken(clientId, clientSecret, this)
        }
...
class SpotifyClientAPI internal constructor(
    ...
    override fun refreshToken(): Token {
        token?.let {
            ...
            if (response.responseCode / 200 == 1) {
                ...
                val processedToken = tempToken.copy(
                        refreshToken = tempToken.refreshToken ?: it.refreshToken,
                        scopes = tempToken.scopes
                )
                this.token = processedToken
        ...
        } ?: throw SpotifyException("No token")
    }
    ...
    fun hasScopes(scope: SpotifyScope, vararg scopes: SpotifyScope): Boolean =
            !isTokenValid(false).isValid && (scopes.toList() + scope).all {
                token?.scopes?.contains(it) ?: false
            }
}
